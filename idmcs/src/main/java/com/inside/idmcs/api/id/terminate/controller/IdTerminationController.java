package com.inside.idmcs.api.id.terminate.controller;
...
import com.inside.idmcs.api.id.terminate.service.IdTerminationService;
...
@Slf4j
@Controller
public class IdTerminationController {
	
private final IdTerminationService idTerminationService;
	
	public IdTerminationController (IdTerminationService idTerminationService) {
		this.idTerminationService = idTerminationService;
	}
	
	/**
	 * 신분 해지 요청을 처리하는 API 엔드포인트.
	 *
	 * 이 메서드는 주어진 IdTerminationReqVO 객체와 HTTP 요청 정보를 사용하여 
	 * 신분 해지를 수행합니다. 성공 시 ResVO 객체를 반환하며, 예외 발생 시 
	 ...
	 * @return 신분 해지 결과를 포함한 ResponseEntity 객체
	 *         - 성공: HTTP 상태 코드 200과 함께 ResVO 객체 반환
	 *         - CustomException 발생 시: 오류 코드와 메시지 포함
	 *         - 기타 예외 발생 시: F901 오류 코드와 메시지 포함
	 */
	@PostMapping("/api/idv/rrc/terminate")
	public <T> ResponseEntity<ResVO<T>> registIdRequest(@Valid @RequestBody IdTerminationReqVO reqVO, HttpServletRequest request) {
		
		try {
			ResVO<T> resVO = idTerminationService.terminateIdRequest(reqVO, request);
